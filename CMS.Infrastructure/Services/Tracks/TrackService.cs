...
namespace CMS.Infrastructure.Services.Tracks
{
    public class TrackService : ITrackService
    {
        ...
        //public async Task<ResponseDto> GetAll(Pagination pagination, Query query)
        //{
        //    var queryString = _db.Tracks.Include(x => x.Category).Include(x => x.PublishedBy).Where(x => !x.IsDelete).AsQueryable();
        //    var dataCount = queryString.Count();
        //    var skipValue = pagination.GetSkipValue();
        //    var dataList = await queryString.Skip(skipValue).Take(pagination.PerPage).ToListAsync();
        //    var tracks = _mapper.Map<List<TrackViewModel>>(dataList);
        ...
        public async Task<int> Update(UpdateTrackDto dto)
        {
            ...
            var tracks = await _db.Tracks.SingleOrDefaultAsync(x => x.Id == dto.Id && !x.IsDelete);
            if (tracks == null)
            {
                throw new EntityNotFoundException();
            }
            var Updatedtracks = _mapper.Map(dto, tracks);
            if (dto.Track != null)
            {
                Updatedtracks.TrackUrl = await _fileService.SaveFile(dto.Track, "Tracks");
            }

            _db.Tracks.Update(Updatedtracks);
            await _db.SaveChangesAsync();
            return tracks.Id;
        }
